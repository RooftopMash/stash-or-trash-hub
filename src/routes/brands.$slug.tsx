...
function getMoodySotGrade(score: number) {
  if (score >= 85) {
    return {
      ...
      description:
        "Prime Trust Grade. Excellent community sentiment, extremely low dissatisfaction.",
    };
  } else if (score >= 70) {
    return {
      grade: "AA",
    ...
    return {
      ...
      description:
        "Vulnerable Grade. High customer service friction and visible negative sentiment.",
    };
  } else {
    return {
      grade: "D (Trash)",
      ...
      description:
        "Substantial Risk. Severe dissatisfaction, action urgently required to restore goodwill.",
    };
  }
}
    ...
    <div className="min-h-screen pb-16">
            ...
            <section className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-6 sm:flex-row shadow-sm">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div
                    className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold border ${getMoodySotGrade(brand.trust_score).color} ${getMoodySotGrade(brand.trust_score).border}`}
                  >
                  ...
                  <div className="space-y-4">
                    {(feed ?? []).length === 0 ? (
                      <p className="text-sm text-muted-foreground py-4 text-center">
                        No social ratings yet. Be the first to Stash or Trash!
                      </p>
                    ) : (
                      (feed ?? []).map((item) => (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/60 pb-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Exclusive rewards directed to verified clients and active raters.
                      </p>
                    </div>
                    <Button
                      size="sm"
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 mt-2">
                    ...
                    <div className="rounded-xl border border-border bg-secondary/20 p-4 flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="mt-2 font-display font-bold text-base text-foreground">
                          Verified Stashers Discount
                        </h4>
                        <p className="text-xs text-muted-foreground mt-1">
                          For active raters who voted "Stash" on {brand.name}. Shows that loyalty
                          deserves real rewards.
                        </p>
                      </div>
                      <div className="mt-4 border-t border-border/50 pt-3 flex items-center justify-between">
                        ...
                        <Button
                          size="sm"
                          onClick={() =>
                            claimCoupon(`SOT_LOYAL_${brand.name.toUpperCase().slice(0, 4)}_20`)
                          }
                          className="gap-1"
                        >
                          {claimedCoupons[
                    ...
                    <div className="rounded-xl border border-border bg-secondary/20 p-4 flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="mt-2 font-display font-bold text-base text-foreground">
                          Brand Recovery Voucher
                        </h4>
                        <p className="text-xs text-muted-foreground mt-1">
                          A customer care signal to rebuild relationships. Available for clients who
                      ...
                      <div className="mt-4 border-t border-border/50 pt-3 flex items-center justify-between">
                        ...
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            claimCoupon(`SOT_CARE_${brand.name.toUpperCase().slice(0, 4)}_10`)
                          }
                  ...
                  <div className="rounded-xl bg-violet-500/5 border border-violet-500/10 p-4 text-xs flex gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground mt-0.5">
                        By rating brands on Stash or Trash, you assist brands with actionable UX/CX
                        data. In turn, brands direct digital rewards and gifts back to the
                        community, establishing the ultimate credible feedback loop.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {activeTab === "moodys" && (
                <div className="p-4 space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      The Moody's of consumer ratings, scaling credibility via decentralized rater
                      feedback.
                    </p>
                  </div>
                  ...
                  <div className="rounded-xl border border-border bg-secondary/10 p-4 flex flex-col sm:flex-row items-center gap-4">
                    <div
                      className={`h-16 w-16 rounded-2xl flex items-center justify-center font-display text-2xl font-black shadow-inner shrink-0 ${getMoodySotGrade(brand.trust_score).color}`}
                    >
                      {getMoodySotGrade(brand.trust_score).grade}
                    </div>
                    <div className="text-center sm:text-left">
                      <h4 className="font-display font-bold text-base">
                        Current Brand Sovereign Grade: {getMoodySotGrade(brand.trust_score).grade}
                      </h4>
                      <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                        {getMoodySotGrade(brand.trust_score).description}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-display font-bold text-sm text-muted-foreground uppercase tracking-wide">
                      Consumer Experience Rating Matrix
                    </h4>
                  ...
                  <div className="border-t border-border/60 pt-4 flex items-center justify-between text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <ThumbsUp className="h-3.5 w-3.5 text-stash" /> Real-time active voting
                    </span>
      ...
      <Dialog open={qrModalOpen} onOpenChange={setQrModalOpen}>
        <DialogContent className="sm:max-w-md bg-card rounded-2xl border border-border">
          ...
          <div className="space-y-4">
            <p className="text-xs text-muted-foreground">
              Direct clients from physical billboards & print ads to brand online e-commerce
              platforms and redeemable Stash or Trash client rewards. Select a simulated billboard
              below to scan:
            </p>
            ...
            <div className="relative aspect-video rounded-xl bg-black border-2 border-indigo-500/20 flex flex-col items-center justify-center overflow-hidden">
              {scanning ? (
                <>
                  ...
                  <span className="mt-2 text-xs font-mono font-bold text-indigo-400 uppercase tracking-widest animate-pulse">
                    Capturing Digital Billboard QR...
                  </span>
                </>
              ) : scannedBillboard ? (
                <div className="p-4 text-center z-10 space-y-2">
                  ...
                  <h4 className="font-display font-extrabold text-sm text-white">
                    Billboard Decoded Successfully!
                  </h4>
                ...
                <div className="text-center p-4">
                  ...
                  <p className="text-[10px] font-mono text-zinc-500 mt-2">
                    Ready. Align digital billboard QR within focus.
                  </p>
                </div>
              )}
            </div>
            ...
            {scannedBillboard && (
              <div className="rounded-xl border border-dashed border-amber-500/30 bg-amber-500/5 p-4 space-y-2 animate-fade-in">
                ...
                <div className="pt-2 flex gap-2">
                  ...
                  <Button size="sm" variant="outline" className="gap-1 font-bold" asChild>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              <label className="text-xs font-bold text-muted-foreground">
                Select a Billboard Advertisement to Mock-Scan:
              </label>
              {BILLBOARDS.map((bb) => (
                <button
                  key={bb.id}
            ...
            <div className="border-t border-border pt-3 flex justify-end">
              <Button size="sm" variant="ghost" onClick={() => setQrModalOpen(false)}>
                Close
              </Button>
            </div>
          </div>
