            ...
            <TabsContent value="importer" className="mt-6 space-y-4">
              <div className="rounded-2xl border border-border bg-card p-5">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Free, CC0-licensed. Pulls companies (with logos where available) for the selected country
                ...
                <div className="mt-4 flex flex-wrap items-end gap-3">
                  ...
                  <label className="flex flex-col text-sm">
                    ...
                    <Input
                      type="number" min={1} max={200} className="w-24"
            ...
            <TabsContent value="queue" className="mt-6 space-y-2">
              {!candidates?.length ? (
                ...
                <div key={c.id} className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4">
                  {c.logo_url ? (
                    <img src={c.logo_url} alt={c.name} className="h-10 w-10 rounded object-contain bg-white" />
                  ) : (
              ...
              {(verifications ?? []).length === 0 ? (
                ...
                <div key={r.id} className="flex items-center justify-between gap-4 rounded-2xl border border-border bg-card p-4">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => review(r.id, r.brand_id, true)}>{t("admin.approve")}</Button>
                    <Button size="sm" variant="outline" onClick={() => review(r.id, r.brand_id, false)}>{t("admin.reject")}</Button>
                  </div>
                </div>
              ))}
            </TabsContent>
