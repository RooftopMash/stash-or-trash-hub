  ...
  return (
    <section className="mt-8 rounded-2xl border border-border bg-card p-5 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-stash/10 text-stash">
          <ShieldCheck className="h-5 w-5" />
        </div>
        <div>
          <h2 className="font-display text-lg font-bold">Verified social activity</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            Connect accounts to prove that your Stashes and Trashes came from a real profile. We
            request read-only access and never publish on your behalf.
          </p>
        </div>
      </div>
          ...
          return (
            <div key={id} className="flex items-center gap-3 rounded-xl border border-border p-3">
              ...
              >
                {connected ? (
                  <>
                    <Unlink2 className="h-3.5 w-3.5" /> Disconnect
                  </>
                ) : (
                  <>
                    <Link2 className="h-3.5 w-3.5" /> {pending ? "Pending" : "Connect"}
                  </>
                )}
              </Button>
            </div>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Instagram, TikTok, X, and additional verification providers can plug into this same secure
        connection layer next.
      </p>
    </section>
  );
}
