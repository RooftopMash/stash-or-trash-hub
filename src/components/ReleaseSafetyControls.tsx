...
export function ReleaseSafetyControls({ userId }: { userId: string }) {
  ...
  async function requestDeletion() {
    ...
    const { error } = await supabase
      ...
      .upsert({ user_id: userId, status: "requested" });
    setDeleting(false);
  ...
  return (
    <section className="mt-6 rounded-2xl border border-border bg-card p-5">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Your consent, reports, blocks, and deletion requests are stored with owner-only access
            controls.
          </p>
        </div>
      </div>
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <a href="/terms" className="rounded-xl border border-border p-3 text-sm hover:border-stash">
          <FileCheck2 className="mb-2 h-4 w-4 text-stash" />
          <strong>Terms</strong>
          <span className="mt-1 block text-xs text-muted-foreground">Version {TERMS_VERSION}</span>
        </a>
        <a
          href="/privacy"
          className="rounded-xl border border-border p-3 text-sm hover:border-stash"
        >
          <ShieldCheck className="mb-2 h-4 w-4 text-stash" />
      ...
      <div className="mt-4 flex items-start gap-3 border-t border-border pt-4">
        ...
        />
        <label htmlFor="release-consent" className="text-xs leading-5 text-muted-foreground">
          I have read and accept the current Terms and Privacy Policy. I understand that public
          submissions may be reviewed for safety, authenticity, and policy compliance.
        </label>
      </div>
      <Button className="mt-3" size="sm" onClick={saveConsent} disabled={!accepted || saving}>
      ...
      <div className="mt-4 grid gap-2 border-t border-border pt-4 text-xs text-muted-foreground sm:grid-cols-3">
        <span className="flex items-center gap-2">
          <Flag className="h-3.5 w-3.5" /> Report harmful content
        </span>
        <span className="flex items-center gap-2">
          <Ban className="h-3.5 w-3.5" /> Block unwanted contact
        </span>
        <span className="flex items-center gap-2">
          <AlertTriangle className="h-3.5 w-3.5" /> Human appeal review
        </span>
      </div>
    </section>
  );
...
export async function reportUGC(
  reporterId: string,
  targetType: "wall_post" | "item" | "comment" | "brand",
  targetId: string,
  reason: string,
  details?: string,
) {
