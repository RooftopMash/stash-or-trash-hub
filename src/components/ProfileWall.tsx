...
import {
  ...
  fetchProfileWallPosts,
  type WallPostType,
} from "@/lib/profile-wall";
...
export function ProfileWall({ profileId, isOwner }: { profileId: string; isOwner: boolean }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (review.error)
        throw new Error(
          "Your post could not be reviewed. Please try again when the safety service is available.",
        );
      if (review.data.decision !== "approved") {
        throw new Error(
          review.data.decision === "blocked"
            ? "This post cannot be published because it failed the safety review."
            : "This post needs human review before it can be published.",
        );
      }
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border border-stash/25 bg-card">
      <div className="border-b border-stash/15 bg-stash/5 p-5">
        ...
        <h2 className="mt-1 font-display text-xl font-extrabold">
          Experiences, ideas and innovations
        </h2>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          A direct channel to brands. Your public record stays visible for CX, PR, and
          accountability.
        </p>
        {isOwner && user && (
          <div className="mt-4 space-y-3">
            ...
            <div className="flex justify-end">
              <Button onClick={publish} disabled={submitting || !body.trim()}>
                <Send className="mr-2 h-4 w-4" />
                {submitting ? "Publishing..." : "Publish to wall"}
              </Button>
            </div>
          </div>
        )}
        <div className="mt-4 grid gap-2 border-t border-stash/15 pt-4 text-xs text-muted-foreground sm:grid-cols-3">
          <span className="flex items-center gap-2">
            <Eye className="h-4 w-4 text-stash" /> Public accountability
          </span>
          <span className="flex items-center gap-2">
            <Send className="h-4 w-4 text-trash" /> Direct to brands
          </span>
          <span className="flex items-center gap-2">
            <LockKeyhole className="h-4 w-4" /> Secure data trail
          </span>
        </div>
      </div>
      <div className="divide-y divide-border">
        {query.isLoading ? (
          <p className="p-5 text-sm text-muted-foreground">Loading wall...</p>
        ) : query.data?.length ? (
          query.data.map((post) => (
            <article key={post.id} className="p-5">
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs font-extrabold uppercase tracking-wider text-stash">
                  {post.post_type}
                </span>
              ...
              <div className="mt-3 flex justify-end">
                <Button
                  ...
                  onClick={async () => {
                    if (!user) return;
                    const result = await reportUGC(
                      user.id,
                      "wall_post",
                      post.id,
                      "community report",
                    );
                    if (result.error) toast.error("Could not submit report");
          ...
          <p className="p-5 text-sm text-muted-foreground">
            No public posts yet. This wall is ready for the first experience or idea.
          </p>
        )}
      </div>
    </section>
