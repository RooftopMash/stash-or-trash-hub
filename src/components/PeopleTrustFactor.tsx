...
export function PeopleTrustFactor({
  signals,
  compact = false,
}: {
  ...
  const scoreColor =
    score >= 75 ? "text-emerald-600" : score >= 55 ? "text-amber-600" : "text-muted-foreground";
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 id="people-trust-factor-title" className="mt-2 font-display text-xl font-bold">
            A transparent people-to-brand signal
          </h3>
        </div>
        <div className="text-right">
          <p className={cn("text-3xl font-extrabold", scoreColor)}>{score}</p>
          <p className="text-xs font-semibold text-muted-foreground">{status}</p>
        </div>
      </div>
      <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
        An internal SOT framework combining evidence quality, brand response, lived experience, and
        trust signals. It is not an external certification or endorsement.
      </p>
      {!compact && (
          ...
          {Object.entries(signals).map(([label, value]) => (
            <div key={label} className="rounded-xl border border-border bg-background p-3">
              ...
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-secondary">
                <div className="h-full rounded-full bg-stash" style={{ width: `${value}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        Scores require sufficient, comparable data and should be read with the supporting evidence.
      </div>
    </section>
  );
}
...
export function PeopleTrustFactorLink() {
  return (
    <span className="inline-flex items-center gap-1 text-xs font-semibold text-stash">
      <BarChart3 className="h-3.5 w-3.5" />
      People Trust framework
    </span>
  );
}
