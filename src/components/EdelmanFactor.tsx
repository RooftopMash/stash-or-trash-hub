...
export function calculateEdelmanFactor(signals: FactorSignals) {
  const values = Object.values(signals);
  const score = Math.round(values.reduce((sum, value) => sum + value, 0) / values.length);
  return { score, signals };
}

export function EdelmanFactor({ signals, compact = false }: { signals: FactorSignals; compact?: boolean }) {
  const { score } = calculateEdelmanFactor(signals);
  ...
  return (
    <section className={cn("rounded-2xl border border-stash/20 bg-stash/5", compact ? "p-4" : "p-5")} aria-labelledby="edelman-factor-title">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-stash"><ShieldCheck className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-[0.16em]">The Edelman FACTOR</span></div>
          <h3 id="edelman-factor-title" className="mt-2 font-display text-xl font-bold">A transparent people-to-brand signal</h3>
        </div>
      ...
      <p className="mt-3 text-sm leading-relaxed text-muted-foreground">An internal SOT framework combining evidence quality, brand response, lived experience, and trust signals. It is not an Edelman endorsement or official Edelman methodology.</p>
...
export function EdelmanFactorLink() {
  return <span className="inline-flex items-center gap-1 text-xs font-semibold text-stash"><BarChart3 className="h-3.5 w-3.5" />Edelman FACTOR framework</span>;
}
