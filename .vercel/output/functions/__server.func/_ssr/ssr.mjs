...
if (typeof globalThis.addEventListener === "function") {
	globalThis.addEventListener("error", (event) => {
		...
		if (errorEvent.error instanceof Error || errorEvent.message) record(errorEvent.error ?? new Error(errorEvent.message));
	});
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-dfXDSG0B.mjs").then((n) => n.t).then((m) => m.default ?? m);
