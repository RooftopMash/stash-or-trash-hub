...
async function queryWikidata(query) {
  ...
  for (let i = 0; i < 4; i++) {
    try {
      const response = await fetch(`${endpoint}?origin=*&format=json&query=${encodeURIComponent(query)}`, { headers: { Accept: "application/sparql-results+json", "User-Agent": "StashOrTrashHub/1.0" } });
...
for (let i = 0; i < countryCodes.length; i += 3) {
  const batch = await Promise.all(countryCodes.slice(i, i + 3).map(importCountry));
  results.push(...batch); console.log(JSON.stringify(batch)); await sleep(1000);
}
