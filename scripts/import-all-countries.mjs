...
const requestedCodes = process.env.COUNTRY_CODES?.split(",")
  ...
  for (let i = 0; i < 4; i++) {
    try {
      const response = await fetch(
        `${endpoint}?origin=*&format=json&query=${encodeURIComponent(query)}`,
        {
          signal: AbortSignal.timeout(30000),
          headers: {
            Accept: "application/sparql-results+json",
            "User-Agent": "StashOrTrashHub/1.0",
          },
        },
      );
...
async function importCountry(code) {
  ...
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error("country timeout")), 90000),
  );
  return Promise.race([importCountryUnbounded(code, country), timeout]);
}

async function importCountryUnbounded(code, country) {
  const queries = classes.flatMap((cls) => [
    query(cls, country, "wdt:P17"),
    query(cls, country, "wdt:P159/wdt:P17"),
  ]);
  const responses = await Promise.allSettled(queries.map(queryWikidata));
  const entities = new Map();
  for (const result of responses)
    for (const row of result.status === "fulfilled" ? result.value : []) {
      const sourceId = row.item?.value?.split("/").pop();
      const name = row.itemLabel?.value;
      if (!sourceId || !name || /^Q\\d+$/.test(name)) continue;
      const current = entities.get(sourceId) ?? {
        source: "wikidata",
        source_id: sourceId,
        name,
        slug: slugify(name),
      ...
      current.category ||= row.industryLabel?.value ?? null;
      current.description ||= row.desc?.value ?? null;
      current.website ||= row.website?.value ?? null;
      current.logo_url ||= row.logo?.value ?? null;
      entities.set(sourceId, current);
    }
  if (!entities.size) {
...
for (let i = 0; i < countryCodes.length; i += 3) {
  const batch = await Promise.all(
    countryCodes.slice(i, i + 3).map(async (code) => {
      try {
        return await importCountry(code);
      } catch (error) {
        return {
          code,
          inserted: 0,
